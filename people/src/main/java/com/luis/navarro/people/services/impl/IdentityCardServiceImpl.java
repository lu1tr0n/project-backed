...
public class IdentityCardServiceImpl implements IdentityCardService {
	...
	public IdentityCardDTO findByNumber(String number) {
		return mapper.convertValue(identityRepo.findByNumber(number).get(), IdentityCardDTO.class);
	}
	...
	public Boolean deleteByIdentityCard(String number) {
		Optional<IdentityCard> deleteIdentity = identityRepo.findByNumber(number);
		
		if (deleteIdentity.isEmpty()) {
			throw new NoFoundDataException("No Found Identity Card.");
		}
		
		identityRepo.delete(deleteIdentity.get());
		return Boolean.TRUE;
	}
